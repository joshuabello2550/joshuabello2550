...
function saveItemToDatabase(post_text, main_topic_text, name_input){
	doc = db.collection("forum_postsACT").add({
		postText: post_text,
...
$("#response_div_form").submit(function(e) {
	...
	doc_replies = db.collection("forum_postsACT").doc(post_id_comment).collection("response").add({
...
function LoopThroughPosts(post_str, topic_str, post_id) {
	db.collection("forum_postsACT").get().then(function(querySnapshot) {
...
function loadComments(post_id_comment, doc) {
	...
	console.log(k);
	console.log('3')
	db.collection("forum_postsACT").doc(post_id_comment).collection("response").get().then(function(querySnapshot) {
