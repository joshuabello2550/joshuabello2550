...
function getRandomX() {
      // Put your code here!
    console.log('pickRandomX');
    return randomX = Math.random() * window.innerWidth;
}

function getRandomY() {
  // Put your code here!
    console.log('pickRandomY');
    return randomY = Math.random() * window.innerHeight;
}
...
function insertWordOnPage(word, x, y){
  // 1) Create an H1 element and set its text to be "word"
    ...
    var w = window.document.createTextNode(word);
    h.appendChild(w);
...
function findMatchingWord(text_input) {
  // Your code here!
    var nodeList = document.getElementById('container').childNodes;

    for (i = 0; i < nodeList.length; i++) {
        var value = nodeList[i].innerHTML;
        if (value === text_input) {
            return nodeList[i];
        }
    }
